...
<body>
   ...
   <section class="dorne-features-events-area bg-img bg-overlay-9 section-padding-100-50" style="background-image: url(img/bg-img/hero-3.jpg)">
        ...
        <br> 
            ...
            $statement = "SELECT ACCOUNT.ID,ACCOUNT.NAME,ACCOUNT.ADDRESS,ACCOUNT.PHONENO,TOURHOSTID.* from TOURHOSTID,ACCOUNT where TOURHOSTID.HOSTID=ACCOUNT.ID and TOURHOSTID.HOSTID=:hostid order by TOURHOSTID.POSTEDDATE desc";
            $s = oci_parse($c, $statement);
            oci_bind_by_name($s, ':hostid', $_SESSION['id']);
            oci_execute($s);
            while (($row = oci_fetch_assoc($s)) != false) 
            {
                ...
                {
                    echo '<tr>
                    <td colspan=2><button type="button'.$tourid.'" id="join" onclick=location.href="deletetourvarification.php?q="+'.$tourid.';>DELETE THIS HOSTED TOUR</button></td>
                    </tr>';
                }
                echo '</table>';
